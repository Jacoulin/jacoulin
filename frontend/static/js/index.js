...
Vue.component('j-dos-list', {
    ...
    template: `
        <div class="j-dos-list">
                    ...
                    <div class="j-dos-item" v-for="dosl in dosls" v-bind:dosl-id="dosl.id" @click="change($event, dosl)">
     ...
     `,
    methods: {
        change: function (e, dos) {
            // this.$emit('print-top', e);
            console.log(dos);
            this.$emit('change', e, dos);
        },
    }
});
...
let main = new Vue({
    ...
    data: {
        ...
        src: `<source v-bind:src="http://music.163.com/song/media/outer/url?id=488249475" type="audio/mpeg" codecs="mp3" />`,
        // dosls: [
        //     {
        //         id: 1,
        //         title: '灰姑娘',
    ...
    methods: {
        ...
        getDuration: function () {
            console.log(this.$refs['j-audio'].duration);
        ...
        getAllDosls: function() {
            axios.get('http://127.0.0.1:8000/song')
                    .then(result=>{
                        console.log(result.data);
                        // console.log(JSON.parse(result));
                        this.dosls = result.data.data;
                        // this.dosls = [{
                        //     name: "dfefefefefefefefe",
                        //     url: "http://music.163.com/song/media/outer/url?id=488249475",
                        //     id: "488249475",
                        //     title: "wwoowwowowowowow",
                        //     duration: 0,
                        //     singer: "null"
                        // }]
                        console.log(this.dosls);
                        console.log(this.dosls[0]);
                        console.log(this.dosls[0]["name"]);
                    })
        },
        ...
        change: function (e, dos){
            ...
            if (this.dosPlay.id !== dos.id) {
                this.dosPlay = dos;
            }

            console.log(this.dosPlay);

            if (this.$refs['j-audio'].paused) {
                this.played = true;
                this.$refs['j-audio'].play();
                console.log(this.$refs['j-audio'].paused);
            } else {
                this.played = false;
                this.$refs['j-audio'].pause();
                console.log("no")
